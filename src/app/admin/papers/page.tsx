...
export default function PaperBuilderPage() {
  ...
  // when set, the form is editing this existing paper id (upsert), not creating
  ...
  useEffect(() => setPapers(getAllPapers()), [cloud, version]);
  ...
  function editPaper(p: Paper) {
    ...
    setMinutes(Math.round(p.durationSec / 60));
    setClassId(p.classId ?? "9");
    setSubjectId(p.subjectId ?? "all");
    setSelected(Object.fromEntries(p.questionIds.map((id) => [id, true])));
  ...
  async function save() {
    ...
    try {
      const paper: Paper = {
        id: editingId ?? `paper-${slug(name)}-${Date.now().toString(36)}`,
        name: name.trim(),
        exam,
      ...
      const r = await addCloudPaper(paper); // upsert by id
      if (r.error) return setMsg(`❌ ${r.error}`);
      const wasEditing = Boolean(editingId);
      reset();
      setMsg(wasEditing ? `✅ Updated "${paper.name}".` : `✅ Created "${paper.name}".`);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="mt-1 text-2xl font-bold">{editingId ? "Edit a paper" : "Build a paper"}</h1>
      ...
      <div className="space-y-4 rounded-2xl border border-border bg-card p-5">
        {editingId && (
          <div className="flex items-center justify-between rounded-lg border border-border bg-background px-3 py-2 text-sm">
            ...
            <button onClick={reset} className="text-muted hover:text-brand">
              Cancel edit
            </button>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <input className={input} type="number" min={1} value={minutes} onChange={(e) => setMinutes(Number(e.target.value))} title="Duration (minutes)" />
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <select className={input} value={subjectId} onChange={(e) => setSubjectId(e.target.value as SubjectId | "all")}>
        ...
        <p className="text-xs text-muted">
          Tick questions to include. Picks are kept as you change the class/subject filter, so a
          paper can mix subjects. Currently selected: {selectedIds.length}.
        </p>

        <div className="max-h-72 space-y-1 overflow-y-auto rounded-lg border border-border p-2">
          {pool.length === 0 && <p className="p-2 text-sm text-muted">No questions for this filter yet.</p>}
          {pool.map((q) => (
            <label key={q.id} className="flex cursor-pointer items-start gap-2 rounded p-2 text-sm hover:bg-background">
              <input
                type="checkbox"
        ...
        <div className="flex items-center gap-3">
          <button onClick={save} disabled={busy} className="rounded-lg bg-brand px-5 py-2 font-semibold text-white disabled:opacity-50">
            {busy ? "Saving…" : editingId ? "Save changes" : "Create paper"}
          </button>
      ...
      <div>
        <h2 className="mb-2 font-semibold">
          Papers <span className="text-muted">({papers.length})</span>
        </h2>
          ...
          {papers.map((p) => (
            <div
              ...
              className={`flex items-center justify-between gap-3 rounded-lg border bg-card p-3 text-sm ${
                editingId === p.id ? "border-brand" : "border-border"
              }`}
            >
              ...
              <div className="flex shrink-0 gap-3">
                <button onClick={() => editPaper(p)} className="text-brand hover:underline">
                  edit
                </button>
                <button onClick={() => remove(p.id)} className="text-rose-500 hover:underline">
                  delete
                </button>
              </div>
            </div>
          ))}
          {papers.length === 0 && <p className="text-sm text-muted">No papers yet.</p>}
        </div>
      </div>
    </div>
  );
